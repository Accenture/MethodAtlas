...
| `methodatlas-emit` | Output emitters (CSV, plain, SARIF, JSON, GitHub annotations, delta) and audit-schema types (`DeltaReport`, `ClassificationOverride`) |
...
| `methodatlas-gui-core` | Swing-free GUI domain types (`AppSettings`, `MethodEntry`, `AnalysisModel`, `AiProfile`) and `AuditWriter` (the audit-trail integrity surface, 99 % line coverage) |
