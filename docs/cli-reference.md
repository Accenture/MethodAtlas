...
| `groq` | Groq cloud inference (Language Processing Unit (LPU) hardware, OpenAI-compatible API) |
...
Runs a full scan and bundles the results into a tamper-evident, self-contained directory. The framework token is one of `ASVS`, `PCI-6.4.1`, `NIST-SSDF-PW.8`, `ISO-27001-8.29` (case-insensitive on input). An unknown token causes the CLI to print an error to stderr and exit with code `2`.

The output directory contains `findings.sarif`, `findings.csv`, optional `overrides.yaml` and `ai-responses.jsonl`, `manifest.sha256`, optional `manifest.sha256.signed`, and `pack-meta.json`. For the full layout and verification one-liner see [Evidence packs](usage-modes/evidence-packs.md).

### `-evidence-pack-dir <path>`

Output directory for the evidence pack. Defaults to `<first-scan-root>/evidence-packs/<canonical-framework>/`.

### `-evidence-pack-overwrite`

Allows the command to reuse an existing output directory. Without this flag a pre-existing directory is treated as an error.
...
Name of an environment variable holding the **full keyring content** (not a path). Intended for CI/CD: store the keyring as a platform secret, expose it as an environment variable, and the key is parsed in memory so it never touches the runner's disk. Takes precedence over `-evidence-pack-keyring` when both are set. See [Evidence packs → CI/CD signing](usage-modes/evidence-packs.md#signing-in-cicd-pipelines).

### `-evidence-pack-key-alias <alias>`

Keyring alias of the signing key. Defaults to the first alias in the keyring. For hybrid signing the format is `classicAlias/pqcAlias`.

### `-evidence-pack-sign-algo <algo>`

Signature algorithm identifier. When omitted, the algorithm is derived from the keyring entry (so a keyring generated by `-gen-signing-key` signs with its own algorithm). Single-algorithm values: `Ed25519`, `Ed448`, `RSA`, `ECDSA`, `SPHINCS+`, `ML-DSA`, `SLH-DSA` (`RSA` uses RSA-PSS SHA-256, `ECDSA` uses P-256). A value of the form `classic+pqc` — for example `Ed25519+SPHINCS+` — selects hybrid signing and requires a `classicAlias/pqcAlias` value for `-evidence-pack-key-alias`.

ZeroEcho keyrings are not password-protected; protect the keyring file with file-system permissions or supply its content through a secret with `-evidence-pack-keyring-env`.

For practical examples grouped by use case, see [CLI Examples](cli-examples.md).

### `-gen-signing-key <keyring-file>`

Generates an asymmetric signing key pair and stores it in a ZeroEcho keyring file for later use with `-evidence-pack-keyring`. This is a standalone utility mode: it does not scan sources. The keyring file is created with owner-only permissions (POSIX `0600`); on file systems without POSIX support (for example Windows) a warning is logged and the operator must restrict access through ACLs.

| Sub-flag | Purpose |
|---|---|
| `-key-alias <alias>` | Base alias for the key pair. Stored as `<alias>.pub` and `<alias>.priv`. Default `methodatlas-signing`. |
| `-key-algo <algorithm>` | Signing algorithm: `Ed25519` (default), `RSA`, `ECDSA`, or `SPHINCS+`. |
| `-overwrite` | Replace an existing entry under the same alias. Without it a collision is an error. |

```bash
./methodatlas -gen-signing-key keys/audit-keyring.txt -key-alias audit
```

Alongside the keyring the command also writes `keys/audit-public.pem` — the public key in standard X.509 PEM form — so a signed manifest can be verified with standard tools such as `openssl` (see [Verifying with standard tools](usage-modes/evidence-packs.md#verifying-with-standard-tools-openssl)).

The keyring holds the private key in clear text. Keep it out of version control, restrict its permissions, and never place it inside an evidence pack. See [Evidence packs → Generating a keyring](usage-modes/evidence-packs.md#generating-a-keyring).
...
| `2` | `-evidence-pack` was given an unrecognised framework token |
| `2` | `-gen-signing-key` was given no keyring path or an unsupported `-key-algo` |
